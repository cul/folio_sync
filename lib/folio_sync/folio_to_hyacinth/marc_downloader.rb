...
class FolioSync::FolioToHyacinth::MarcDownloader
  ...
  # AND were modified since the given `modified_since` time.
  ...
  def download_965hyacinth_marc_records(last_x_hours = nil)
    ...
    puts "Downloading MARC records with 965hyacinth modified since: #{modified_since_utc || 'the beginning of time'}"

    @folio_client.find_source_marc_records(modified_since_utc) do |parsed_record|
  ...
  def save_marc_record_to_file(marc_record)
    ...
    puts "Saving MARC record with 001=#{filename} to #{file_path}"
